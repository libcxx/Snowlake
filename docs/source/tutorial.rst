...
The synthesized C++ code makes use of `std::error_code` to handle
errors through the inference deduction process. Therefore,
the Snowlake compiler will also synthesize an extra `InferenceErrorDefn.h`
and `InferenceErrorDefn.cpp` file that contain the error definitions.
